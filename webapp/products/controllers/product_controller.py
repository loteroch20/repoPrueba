...
def get_products():
    ...
    result = [{'id': product.id, 'name': product.name, 'typee': product.typee, 'price': product.price, 'due_date': product.due_date} for product in products]
...
def get_product(product_id):
    ...
    return jsonify({'id': product.id, 'name': product.name, 'typee': product.typee, 'price': product.price, 'due_date': product.due_date})

@product_controller.route('/api/products', methods=['POST'])
def create_product():
    print("creando producto")
    data = request.json
    new_product = Products(name=data['name'], typee=data['typee'], price=data['price'], due_date=data['due_date'])
